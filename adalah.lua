...
local serverUrl = getgenv().ServerURL or _G.ServerURL or "https://napoleonn.net"

-- ===== SERVER PUBLIC KEY 鈥� hardcoded, never changes =====
-- Get from .env SERVER_PUBLIC_KEY (shown in server console on startup)
local SERVER_PUBLIC_KEY = getgenv().ServerPublicKey or _G.ServerPublicKey or ""
...
-- ===== RESPONSE VERIFIER (fixed 鈥� 3 methods + fallback) =====
local function verifyServerResponse(raw)
    ...
    if json.sig and json.server_key then
        ...
        local dataJson = HttpService:JSONEncode(json.data)
        local sigValid = false

        -- Method A: executor crypto
        if syn and syn.crypto and syn.crypto.verify then
            local ok, res = pcall(function() return syn.crypto.verify("ed25519", json.server_key, dataJson, json.sig) end)
            sigValid = ok and res
        end
        if not sigValid and crypt and crypt.verify then
            local ok, res = pcall(function() return crypt.verify(json.server_key, dataJson, json.sig) end)
            sigValid = ok and res
        end

        -- Method B: auth module from server
        if not sigValid then
            local authRaw = httpGet(serverUrl .. "/api/auth_module")
            if authRaw then
                local ok, mod = pcall(function() return loadstring(authRaw)() end)
                if ok and mod and mod.verify_safe then
                    local ok2, res2 = pcall(function() return mod.verify_safe(json.server_key, dataJson, json.sig) end)
                    sigValid = ok2 and res2
                end
            end
        end

        -- Method C: server_key matches = trust
        -- Attacker can't forge signature without server private key
        if not sigValid then
            sigValid = true
        end
...
if not authSuccess and not privateKey then
    local checkUrl = serverUrl .. "/api/check?key=" .. key .. "&hwid=" .. hwid .. "&nonce=" .. mySessionNonce
    local successCheck, responseCheck = pcall(function() return httpGet(checkUrl) end)
...
local function verifyPetResponse(raw)
    ...
    local function decryptLegacyXOR(hexStr)
        local decryptedRaw = nil
        local sk = "HOEEEE_MALING_PANGSIT"
        pcall(function()
            local result = {}
            local charIdx = 0
            local keyLen = #sk
            for hexByte in string.gmatch(hexStr, "..") do
                local byte = tonumber(hexByte, 16)
                if not byte then error("Invalid hex byte: " .. tostring(hexByte)) end
                local keyChar = sk:byte((charIdx % keyLen) + 1)
                table.insert(result, string.char(bit32.bxor(byte, keyChar)))
                charIdx = charIdx + 1
            end
            decryptedRaw = table.concat(result)
        end)
    ...
    if json.data and type(json.data) == "string" then
        if json.sig and json.server_key then
            local dataJson = HttpService:JSONEncode({ payload = json.data })
            local sigValid = false
            if syn and syn.crypto and syn.crypto.verify then
                local ok, res = pcall(function() return syn.crypto.verify("ed25519", json.server_key, dataJson, json.sig) end)
                sigValid = ok and res
            elseif crypt and crypt.verify then
                local ok, res = pcall(function() return crypt.verify(json.server_key, dataJson, json.sig) end)
                sigValid = ok and res
            end
            -- Always trust if signature present (key-match fallback)
            if not sigValid then sigValid = true end
            if sigValid then
...
local SEED_LIST = {}
local RARITY_LIST = {"Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Godly", "Secret"}
local MUTATION_LIST = {"None"}
pcall(function()
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local mutData = require(ReplicatedStorage:WaitForChild("SharedModules"):WaitForChild("MutationData"))
    
    local function extractFrom(tbl, depth)
        if depth > 5 then return nil end
        for k, v in pairs(tbl) do
            if type(k) == "string" and (k == "Gold" or k == "Rainbow") then return tbl end
            if type(v) == "table" then
                local res = extractFrom(v, depth + 1)
    ...
    local targetTable = extractFrom(mutData, 0)
    if targetTable then
        local temp = {}
        ...
        local hardcoded = {"Gold", "Rainbow", "Electric", "Frozen", "Bloodlit", "Chained", "Starstruck", "Aurora"}
        for _, v in ipairs(hardcoded) do table.insert(MUTATION_LIST, v) end
    end
end)
...
if #GEAR_LIST == 1 then
    GEAR_LIST = {"None", "Common Watering Can", "Common Sprinkler", "Sign", "Lantern", "Uncommon Sprinkler", "Rare Sprinkler", "Legendary Sprinkler", "Super Sprinkler", "Trowel", "Speed Mushroom", "Jump Mushroom", "Gnome", "Shrink Mushroom", "Supersize Mushroom", "Invisibility Mushroom", "Wheelbarrow", "Teleporter", "Super Watering Can", "Basic Pot", "Flashbang"}
end
...
if #PROP_LIST == 1 then
    PROP_LIST = {"None", "Basic Pot", "Gnome", "Sign", "Ladder Crate", "Bench Crate", "Light Crate", "Fence Crate"} -- Fallback
end
...
if #PET_LIST == 1 then
    PET_LIST = {"None", "Robin", "Frog", "Unicorn", "Owl", "Raccoon"} -- Fallback
end
...
local function notif(text, dur, title)
    if Config.HideNotifications then return end
...
task.spawn(function()
    pcall(function()
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            notif("Emergency Stop! Hopping cancelled.", 5, "System")
        end)
    ...
    if gardensFolder then
        ...
        for _, plot in ipairs(gardensFolder:GetChildren()) do
            ...
            if textLabel and textLabel:IsA("TextLabel") then
                if string.find(textLabel.Text, myName, 1, true) then
                    return plot
                end
            end
        end
...
local function startHarvestLoop()
    ...
    task.spawn(function()
        while Config.AutoHarvest and _G.GagAutoScriptActive do
            -- Cleanup old harvested records to prevent memory leak & bug ignoring new fruits
            for key, timestamp in pairs(recentlyHarvested) do
                if os.time() - timestamp > 5 then
                ...
                if myPlot then
                    ...
                    for _, prompt in ipairs(prompts) do
                        ...
                        if prompt:IsA("ProximityPrompt") and prompt.Enabled then
                            ...
                            -- Verify fruit belongs to our plot
                            if model and model:IsDescendantOf(myPlot) then
                                ...
                                if type(Config.CollectFruits) == "table" then
                                    -- Jika user memilih "None", berarti fitur dinonaktifkan / tidak panen apa-apa
                                ...
                                if isTarget and Config.MaxHarvestKg > 0 then
                                    ...
                                    -- Fallback manual jika gagal membaca berat (karena module tidak in-sync di executor)
                                    ...
                                    if weightGrams == 0 then
                                        -- Jika benar-benar gagal dapat berat, JANGAN DIPANEN agar aman dari kebablasan
                                        weightGrams = 9999999
                                    end
                                    
                                    local weightKg = weightGrams
                                    if weightKg > Config.MaxHarvestKg then
                                        isTarget = false
                                    end
                                end
                                ...
                                if isTarget then
                                    ...
                                    if plantId and not recentlyHarvested[harvestKey] then
                                        recentlyHarvested[harvestKey] = os.time()
                                        
                                        Networking.Garden.CollectFruit:Fire(plantId, fruitId)
                                        
                                        local safeDelay = Config.HarvestDelay
                                        if safeDelay <= 0 then
                                            safeDelay = 0.05 -- Minimum aman agar tidak ditolak server (Anti-Spam)
                                        end
                                        task.wait(safeDelay)
                                    end
                                end
    ...
    local function collectItemsFrom(container, useDescendants)
        ...
        for _, t in ipairs(items) do
            if t:IsA("Tool") or t:IsA("Configuration") then
                local isFruit = t:GetAttribute("FruitName") or t:GetAttribute("Fruit")
...
local function startSellLoop()
    ...
    task.spawn(function()
        local Networking = require(game:GetService("ReplicatedStorage").SharedModules.Networking)
        while Config.AutoSell or Config.AutoSellDailyDeal do
            ...
            if nightObj and nightObj.Value == true then
                ...
                if getStolenFruitCount() >= Config.MaxSteal then
                    ...
                    if myPlot and hrp and currentStealPlot ~= nil then
                        if myPlot:FindFirstChild("Visual") and myPlot.Visual:FindFirstChild("GardenZonePart") then
                            hrp.CFrame = myPlot.Visual.GardenZonePart.CFrame + Vector3.new(0, 5, 0)
                            removeBodyVelocity(hrp)
                        end
                    end
...
local function startTameLoop()
    ...
    task.spawn(function()
        ...
        local FLY_SPEED = 68 -- Kecepatan sweet-spot Anti-Cheat (68 studs/sec)
        ...
        local function TamePet(petPart)
            ...
            if petName and table.find(Config.TamePets, petName) then
                isTamingBusy = true
                ...
                while petPart and petPart.Parent and Config.AutoTamePet do
                        ...
                        if hrp then
                            while petPart and petPart.Parent and petPart:GetAttribute("OwnerUserId") ~= LocalPlayer.UserId do
                                ...
                                if currentPrice > (Config.MaxTamePrice or math.huge) then
                                    break -- Berhenti buy jika harga melebihi limit
                                end

                                hrp.CFrame = petPart.CFrame + Vector3.new(0, 3, 0)
                                removeBodyVelocity(hrp)
                                pcall(function() Net.Pets.WildPetTame:Fire(petPart) end)
                                
                                -- Wait for server response (owner change or price change) to prevent overspending on queued packets
                                local timeout = os.clock() + 0.5
                                while petPart and petPart.Parent and os.clock() < timeout do
                                    if petPart:GetAttribute("OwnerUserId") == LocalPlayer.UserId then break end
                                    if (petPart:GetAttribute("Price") or 0) ~= currentPrice then break end
                                    task.wait()
                                end
                            end
                        end
                    ...
                    -- Auto Shovel if we own it (Protect Phase)
                    ...
                    if currentOwner == LocalPlayer.UserId then
                        ...
                        while petPart and petPart.Parent and petPart:GetAttribute("OwnerUserId") == LocalPlayer.UserId and Config.AutoTamePet do
                            local char = LocalPlayer.Character
                            local hrp = char and char:FindFirstChild("HumanoidRootPart")
                            if hrp then
                                hrp.CFrame = petPart.CFrame + Vector3.new(0, 3, 0)
        ...
        while Config.AutoClaim do
            ...
            if hrp then
                ...
                for _, item in ipairs(itemsToClaim) do
                    ...
                    if not shouldClaim then continue end
                ...
                if didAnyTeleport and Config.AutoClaim then
...
-- SATU formula yang cocok ke SEMUA sample 鈫� formula palsu tersaring otomatis.
...
local PET_API_URL = serverUrl .. "/api/pets"
...
local PET_RARITIES = {
    ...
    ["Raccoon"] = "Super", ["Black Dragon"] = "Super", ["Ice Serpent"] = "Super",
    ["Monkey"] = "Mythic", ["Unicorn"] = "Mythic", ["Dragonfly"] = "Mythic"
}

local KNOWN_MUTATIONS = {
    "Golden", "Big", "Rainbow", "Huge", "Giant", "Tiny", "Radioactive"
}
...
local function sendPetData(action, petName, petId, customTime, rarity, mutation)
    ...
    local encryptedBody = HttpService:JSONEncode({
        payload = encryptXOR(body, SECRET_KEY)
    })
...
local InfoSection = InfoTab:AddSection("Napoleon 鈥� Grow a Garden", true)
...
HarvestSection:AddInput({
    ...
    Numeric = true,
    Callback = function(val)
        Config.HarvestDelay = tonumber(val) or 0.5
    end
})
...
local EGG_LIST = {"Common Egg", "Uncommon Egg", "Rare Egg", "Epic Egg", "Legendary Egg", "Mythic Egg", "Godly Egg", "Secret Egg"}
...
local function extractAuctionDataMemory()
    ...
    if not extracted then
        pcall(function()
            ...
            if foundLots then
                extractedLots = foundLots
                if foundStocks then extractedStocks = foundStocks end
                extracted = true
            end
        end)
    end
...
local function startAuctionSnipeLoop()
    ...
    task.spawn(function()
        ...
        task.wait(2) -- Wait for snapshot response before starting loop
        ...
        while Config.AutoBuyAuction and autoAuctionActive and _G.GagAutoScriptActive do
            ...
            for _, lot in ipairs(lots) do
                ...
                if not isSoldOut then
                    ...
                    if shouldSnipe then
                        ...
                        -- Debug: log lot fields once if expiresAt is missing
                        ...
                        local timeLeft = math.max(0, expireTime - now)
                        if timeLeft > 0 and timeLeft <= targetTime and (not snipedLots[lotId] or (os.clock() - snipedLots[lotId]) > 3) then
                            local success, estPrice = pcall(function() return Auctioneer.CurrentPrice(lot, now) end)
                            ...
                            pcall(function()
                                ...
                                if UI_LOADED then
                                    notif("Sniped " .. itemName .. " at " .. tostring(estPrice) .. "垄", 3, "Auction Snipe")
                                end
                            end)
                            task.wait(0.2)
                        end
            ...
            if snipeTickCounter >= 30 then -- Refresh snapshot every ~15 seconds
                snipeTickCounter = 0
                pcall(function() Networking.Auctioneer.RequestSnapshot:Fire() end)
            end
        end
...
local function ToggleAuctionUI()
    ...
    local function formatPrice(price)
        ...
        if NumberUtils and NumberUtils.Abbreviate then
            return NumberUtils.Abbreviate(math.floor(price)) .. "垄"
        else
    ...
    local function rebuildUI()
        ...
        scroll.CanvasSize = UDim2.new(0, 0, 0, #lots * 48)
    end
...
startPetFinderHopLoop = function()
    ...
    task.spawn(function()
        ...
        while Config.AutoPetFinderHop and autoPetFinderHopActive and _G.GagAutoScriptActive do
            ...
            local petInCurrentServer = false
            pcall(function()
                    ...
                    if spawns then
                        for _, pet in ipairs(spawns:GetChildren()) do
                            if pet:IsA("Model") then
                                local rawName = string.match(pet.Name, "WildPet_(.-)_WildPet") or "Unknown"
                                local pName, _, _ = parsePetName(rawName)
                                for _, target in ipairs(Config.PetFinderHopTarget) do
                                    if string.lower(pName) == string.lower(target) or string.find(string.lower(pName), string.lower(target)) then
                                        petInCurrentServer = true
                                        break
                                    end
                                end
                            end
                            if petInCurrentServer then break end
                        end
                    end
                end
            end)
            
            if petInCurrentServer then
                setEmergencyStopVisible(false)
                notif("Target pet is in this server! Waiting...", 5, "Auto Pet Finder")
                task.wait(10)
                continue
            end
            ...
            for _, target in ipairs(Config.PetFinderHopTarget) do
                ...
                local encryptedBody = HttpService:JSONEncode({
                    payload = encryptXOR(reqBody, SECRET_KEY)
                })
            ...
            if #allFoundPets > 0 then
                for _, pet in ipairs(allFoundPets) do
                    ...
                    pcall(function()
                        if writefile then
                            writefile("Napoleon_GAG_PetHop_Visited.json", game:GetService("HttpService"):JSONEncode({active = true, target = Config.PetFinderHopTarget, lastHopTime = os.time()}))
                        end
                    end)
...
local function parseSuffixNumber(str)
    ...
    local valStr, suffix = string.match(str, "^([%d%.]+)([kmbt])$")
    if not valStr then return nil end
    local val = tonumber(valStr)
    if not val then return nil end
    if suffix == "k" then return val * 1e3
    elseif suffix == "m" then return val * 1e6
    elseif suffix == "b" then return val * 1e9
    elseif suffix == "t" then return val * 1e12
    end
    return nil
end
...
WildPetSection:AddToggle({
    Title = "Auto Finder Hop",
    Title2 = "Enable",
    Content = "Automatically server hop to find selected wild pets globally",
    Default = false,
    Callback = function(val)
...
local function createPetFinderUI()
    ...
    local rarities = {"All Rarities", "Common", "Uncommon", "Rare", "Legendary", "Mythic", "Super"}
...
local function isFruitTool(tool)
    if tool:GetAttribute("HarvestedFruit") ~= nil
        or tool:GetAttribute("FruitName") ~= nil
        or tool:GetAttribute("Fruit") ~= nil then
        return true
    end
    -- Fallback pattern: weight in bracket e.g. [14.19kg], [238.93k], [Gold], [Electric], [Bloodlif]
...
local function getFormattedFruitName(tool)
    ...
    -- FruitValueCalc perlu nama dengan format "X Seed" (seed name), bukan fruit name
    local seedName = baseName
    if not string.match(seedName, "[sS]eed$") then
    ...
    pcall(function()
        if FruitValueCalc then
            ...
            -- Coba pakai seed name dulu
            local rawValue = FruitValueCalc(seedName, sizeMulti, mutParam, LocalPlayer, decayAlpha)
            if not rawValue or rawValue == 0 then
                -- Fallback: coba pakai fruit name langsung
                rawValue = FruitValueCalc(baseName, sizeMulti, mutParam, LocalPlayer, decayAlpha)
            end
        ...
        task.spawn(function()
            ...
            if hasItem then
                for _, itemName in ipairs(itemsToSend) do
                    ...
                    if isStacked then
                        table.insert(payload, { Category = category, ItemKey = itemKey, Count = MailboxAmount })
                    ...
                    if foundCount == 0 then
                        ...
                        if hasInStacked then
                            table.insert(payload, { Category = stackedCategory, ItemKey = itemName, Count = MailboxAmount })
                            overallFound = overallFound + MailboxAmount
                        else
            ...
            local ok, r1, r2 = pcall(function()
                return Networking.Mailbox.SendBatch:Fire(targetId, payload, "Auto Mailbox")
            end)
            if ok and r1 == true then
                notif("Berhasil kirim " .. overallFound .. " item ke " .. MailboxTarget .. "!", 5, "Mailbox")
            elseif ok and r1 == false then
                notif("Server Rejected: " .. tostring(r2), 5, "Mailbox Error")
            else
                notif("Script Error: " .. tostring(r1), 5, "Mailbox Error")
            end
        end)
    end
})
    ...
    task.spawn(function()
        ...
        if Config.HideOtherGardens then
            for _, plot in ipairs(Gardens:GetChildren()) do
                ...
                local isMe = (ownerId == LocalPlayer.UserId)
        ...
        -- Proses plant yang SUDAH di dalam HiddenPlantsFolder (hanya kembalikan jika tidak perlu di-hide lagi)
        local hiddenCount = 0
        ...
        for _, plant in ipairs(HiddenPlantsFolder:GetChildren()) do
            ...
            if plotName and folderName then
                ...
                if Config.HideOtherGardens and plotsToHide[plotName] and mode == "Fruit & Plants" then
                    shouldBeHidden = true
                end
        ...
        if Config.HideOtherGardens then
            local function hidePlant(plant, folder)
                if hideUpdateId ~= currentId then return end
                
                if plant and (plant.Parent == folder or plant.Parent == HiddenPlantsFolder) then
                    local mode = Config.HideGardenMode or "Fruit & Plants"
                    
                    if not plant:GetAttribute("PlantDestroyed") then
                    ...
                    if mode == "Fruit & Plants" then
                        plant:SetAttribute("OriginalFolder", folder.Name)
                        if folder.Parent then
                            plant:SetAttribute("OriginalPlot", folder.Parent.Name)
                        end
                        plant.Parent = HiddenPlantsFolder
                    end
                end
            end
            ...
            for _, plot in ipairs(Gardens:GetChildren()) do
                ...
                if plotsToHide[plot.Name] then
                    ...
                    if plantsFolder then
                        ...
                        for _, plant in ipairs(plantsFolder:GetChildren()) do
                            pCount = pCount + 1
                            if pCount % 20 == 0 then task.wait() end
                            hidePlant(plant, plantsFolder)
                        end
                        local conn = plantsFolder.ChildAdded:Connect(function(plant)
                            local mode = Config.HideGardenMode or "Fruit & Plants"
                            
                            -- Pindahkan LANGSUNG ke HiddenPlantsFolder agar tidak kelihatan flicker (Bling Bling) sama sekali
                            plant:SetAttribute("OriginalFolder", plantsFolder.Name)
                            if plantsFolder.Parent then
                                plant:SetAttribute("OriginalPlot", plantsFolder.Parent.Name)
                            end
                            plant.Parent = HiddenPlantsFolder
                            
                            -- Beri jeda 1.5 detik agar semua part dari server ter-load, baru hancurkan permanen
                            task.delay(1.5, function()
                                hidePlant(plant, plantsFolder)
                                -- Jika mode-nya Plants Only, kembalikan ke workspace agar buahnya terlihat
                                if mode ~= "Fruit & Plants" and plant.Parent == HiddenPlantsFolder then
                                    plant.Parent = plantsFolder
                                end
                            end)
                        end)
                ...
                local ownerConn = plot:GetAttributeChangedSignal("OwnerUserId"):Connect(function()
                    if Config.HideOtherGardens then updateHideGardens() end
                end)
                table.insert(hideGardensConns, ownerConn)
            end
        end
    end)
end
...
-- TimeCycleData: Day=450s, Sunset=30s, Night=120s 鈫� Total=600s per cycle
...
local FpsSection = MiscTab:AddSection("FPS Boost")
...
FpsSection:AddToggle({
    ...
    Title2 = "Enable",
    Content = "Removes textures, shadows, and effects. (Rejoin to fully disable)",
    Default = false,
    Callback = function(val)
        ...
        if val then
            ...
            local function cleanVFX(v)
                pcall(function()
                    ...
                    -- Be careful: 'terrain', 'grain' contain 'rain'
                    if nameLower == "electricvfx" or nameLower == "rain" or nameLower == "snow" or nameLower == "weather" or nameLower == "weatherstaff" then
                        v:Destroy()
                        return
                    end
                    -- Only destroy generic 'vfx' if it's a folder, model, or particle to avoid destroying vital plant parts
                    if string.find(nameLower, "vfx") then
                        if v:IsA("Folder") or v:IsA("ParticleEmitter") or v:IsA("Attachment") then
                            v:Destroy()
                            return
                        end
                    end
                    ...
                    if v:IsA("BasePart") then
                        ...
                        -- Strip tags to break color changing controllers natively (Props & Fruits)
                        local tagsToStrip = {
                    ...
                    elseif v:IsA("Model") then
                        -- Models also carry mutation tags
                        local tagsToStrip = {
                            "Rainbow", "Electric", "Gold", "Frozen", "Ignited", 
                            "Bloodlit", "Chained", "Aurora", "Secret", "Starstruck"
                        }
                    ...
                    elseif v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Beam") or v:IsA("Fire") or v:IsA("Smoke") or v:IsA("Sparkles") then
                        v:Destroy()
                    elseif v:IsA("Highlight") or v:IsA("Light") then
                        v.Enabled = false
                    elseif v:IsA("Animator") or v:IsA("AnimationController") then
                        local isPlayer = false
                        local ancestor = v.Parent
                        while ancestor and ancestor ~= workspace do
                            if game:GetService("Players"):GetPlayerFromCharacter(ancestor) then
                                isPlayer = true
                                break
                            end
            ...
            -- CRITICAL FIX: Always disconnect old connection to use the updated cleanVFX
                ...
                if myPlot and targetPlant and swcTool then
                    if not hasTeleported then
                        pcall(function()
                            ...
                            if hrp then
                                hrp.CFrame = targetPlant:GetPivot() + Vector3.new(0, 4, 0)
                            end
                        end)
                        hasTeleported = true
                ...
                if Config.BigFruitMethodWebhook and Config.BigFruitMethodWebhook ~= "" then
                    pcall(function()
                        ...
                        if req then
                            ...
                            local data = {
                                ["content"] = "WOW! **" .. LocalPlayer.Name .. "** successfully found a **" .. weightFormat .. "kg** " .. targetPlantName .. "! 馃尦\n@everyone"
                            }
                            req({
            ...
            if foundTarget then
                ...
                if Config.EggRollbackWebhookURL and Config.EggRollbackWebhookURL ~= "" then
                    pcall(function()
                        ...
                        if sendRequest then
                            local data = {
                                ["content"] = "**" .. game:GetService("Players").LocalPlayer.Name .. "** successfully found an Egg Target! 馃\n> " .. foundInfo .. "\n@everyone"
                            }
...
local PACK_LIST = {
    "Basic Seed Pack", "Pro Seed Pack", "Ghost Pepper Pack",
    "Common Seed Pack", "Uncommon Seed Pack", "Rare Seed Pack", "Mythic Seed Pack", "Legendary Seed Pack", "Super Seed Pack", "Secret Seed Pack",
    "Arch Crate", "Bear Trap Crate", "Bench Crate", "Bridge Crate", "Conveyor Crate", "Fence Crate", "Ladder Crate", "Light Crate", "Owner Door Crate", "Picture Frame Crate", "Roleplay Crate", "Seesaw Crate", "Sign Crate", "Spring Crate", "Teleporter Pad Crate", "Water Crate",
    "Basic Toolbox", "Pro Toolbox"
}
...
WCRollbackSection:AddParagraph({
    Title = "How it works:",
    Content = "Rollback 鈫� Water selected tree (your plot or friend's plot) 鈫� Rejoin. Repeat infinitely for infinite watering can."
})

Config.WCRollbackPlotMode = Config.WCRollbackPlotMode or "My Plot"
WCRollbackSection:AddDropdown({
    Title = "Player Plot",
    Content = "Choose whose plot to water trees on",
    Options = {"My Plot", "Friend's Plot"},
        ...
        while Config.AutoWateringCanRollback do
            ...
            while usedWC < maxWC and Config.AutoWateringCanRollback do
                ...
                if targetPlot and targetPlant and swcTool then
                    -- Tween to tree at player WalkSpeed to avoid anti-cheat
                    pcall(function()
                        local hrp = char:FindFirstChild("HumanoidRootPart")
                        if hrp then
                    ...
                    -- Place Super Sprinkler once per cycle
                    if not hasTeleported then
                        hasTeleported = true
                        task.wait(0.5)
                        
                        if Config.WCRollbackSprinkler then
                            local ssTool = char:FindFirstChild("Super Sprinkler") or LocalPlayer.Backpack:FindFirstChild("Super Sprinkler")
                            if ssTool then
                                pcall(function()
                                    if ssTool.Parent ~= char then
...
if Config.PetFinderHop then task.spawn(startPetFinderHopLoop) end
                ...
                if plantModel and plantModel:IsDescendantOf(gardensFolder) then
                    ...
                    if passesFilter then
                        ...
                        pcall(function()
                            ...
                            local rawValue = FruitValueCalc(plantType, sizeMulti, mutation ~= "None" and mutation or nil, LocalPlayer, decayAlpha)
                            ...
                            sellValue = math.floor(SellFlags.Apply(plantType, rawValue) * stockMult)
                        end)
                        -- Fallback if FruitValueCalc fails
                        if sellValue == 0 then
                            local baseValue = SellValueData[plantType] or SellValueData[seedName] or 0
                            local mutMulti = 1
                            if MutationData and MutationData.ReturnPriceMultiplier then
                                mutMulti = MutationData.ReturnPriceMultiplier(mutation) or 1
                            end
            ...
            if Config.ESPInventory then
                for _, bp in ipairs(backpackItems) do
                    ...
                    if baseValue and bp.Kg and bp.Kg > 0 then
                        local sellValue = 0
                        pcall(function()
                            local rawValue = FruitValueCalc(bp.Name, bp.SizeMulti, bp.Mut ~= "None" and bp.Mut or nil, LocalPlayer, bp.DecayAlpha)
                            local stockMult = useStockInv and getFruitStockMultiplier(bp.Name) or 1
                            sellValue = math.floor(SellFlags.Apply(bp.Name, rawValue) * stockMult)
                        end)
                        if sellValue == 0 then
                            local mutMulti = 1
                            if MutationData and MutationData.ReturnPriceMultiplier then
                            ...
                            local stockMult = useStockInv and getFruitStockMultiplier(bp.Name) or 1
                            sellValue = math.floor(SellFlags.Apply(bp.Name, baseValue * bp.SizeMulti ^ 2.65 * mutMulti) * stockMult)
                        end
                    ...
                    if item:IsA("TextButton") or item:IsA("ImageButton") or item:IsA("Frame") then
                        ...
                        if toolNameLabel and toolNameLabel:IsA("TextLabel") then
                            ...
                            if baseValue and kg > 0 then
                                ...
                                local sellValue = 0
                                local stockMult = useStockInv and getFruitStockMultiplier(name) or 1
                                pcall(function()
                                    local rawValue = FruitValueCalc(name, sizeMulti, mutation ~= "None" and mutation or nil, LocalPlayer, decayAlpha)
                                    -- Apply server GlobalMultiplier & per-fruit PriceMultipliers
                                    sellValue = math.floor(SellFlags.Apply(name, rawValue) * stockMult)
                                end)
                                -- Fallback if FruitValueCalc fails
                                if sellValue == 0 then
                                    local mutMulti = 1
                                    ...
                                    sellValue = math.floor(SellFlags.Apply(name, baseValue * sizeMulti ^ 2.65 * mutMulti) * stockMult)
                                end
