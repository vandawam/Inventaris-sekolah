<div class=" w-full flex justify-between px-7 py-5 border-b-2 border-blue-200 shadow-xl  {{ Auth::check() && Auth::user()->role == 'teknisi' ? 'bg-yellow-400' : 'bg-blue-800' }}">
    ...
    @if (Auth::check() && Auth::user()->role == 'teknisi')
        ...
        <div class=" flex gap-10">
            ...
            @endif
            @if (Auth::check())
                <div class=" {{ Auth::check() && Auth::user()->role == 'teknisi' ? 'text-yellow-400' : 'text-blue-800' }} bg-white rounded-md px-2 py-1 font-bold text-lg border-b-2 border-white ">
